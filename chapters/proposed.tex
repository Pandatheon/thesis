...
In this chapter, we turn our attention to the QPs with indefinite Hessian matrix, namely indefinte QPs.
The chapter starts with a brief classification of the Hessian regularization in \ref{sec:hessian_reg},
then we introduce the concept of Riccati-based primal regularization in \ref{sec:riccati_primal_reg},
the Riccati-based primal regularization method is consecutively introduced based on an analysis of failing on minimum perturbation in \ref{sec:fail_minimal_perturbation}.
We conclude the chapter with a proof of the asymptotic exactness of the proposed Riccati-based primal regularization method.

\section{Primal regularization}
\label{sec:hessian_reg}
In indefinite QPs, the regularization needed to be applied to the Hessian matrix to ensure a descent search direction in the interior point method.
...
\begin{equation}
    ...
    \end{bmatrix},
\end{equation}
where $E$ is the adjustment matrix for ensuring a positive definite Hessian.
This process is commonly referred to as primal regularization and this section will briefly introduce the two types of primal regularization, 
i.e. the regularization on full Hessian $\tilde{H}$ and the regularization on reduced Hessian $\tilde{M}$.

\subsection{Regularization Strategies}
\label{subsec:reg_strategies}
We would like to start with the numerical techniques used by both full Hessian regularization and reduced Hessian regularization by regarding the calculation of adjustment matrix $E$.

There are several strategies for constructing the adjustment matrix $E$ to ensure a positive definite Hessian, commonly, it can be computed from
\begin{itemize}
    \item Eigenvalue modification (EM): modifies the negative eigenvalues of $\tilde{H}$ \cite{nocedal2006numerical}.
    \item Modified Cholesky factorization (MC): modifies the inertia of $\tilde{H}$ \cite{mcsweeneyModifiedCholeskyDecomposition2017}.
    \item Levenberg-Marquardt (LM): adds a scaled identity matrix to $\tilde{H}$ \cite{More1978levenberg}.
\end{itemize}

We will mainly apply four commonly used strategies for regularization in this work, 
two methods for EM (PROJECT, MIRROR) \cite{verschueren2017sparsity}, one method for MC (GMW) \cite{gill1981practical}, and one method for LM (Inertia Correction) \cite{wachter2006implementation}.

Assuming a matrix $A$ can be eigenvalue decomposed as $A = V D_E V^\top$ and Cholesky factorized as $A = L D_I L^\top$, 
...
\begin{itemize}
    ...
    \end{equation*}
    Where the $\max$ operator is applied component-wise. This approach clips any negative or near-zero eigenvalues to a small positive constant $\epsilon > 0$,
    which keeps a minimal adjustment to the original matrix for positive definiteness, but may lead to an increase in the condition number if the original eigenvalues were not close to zero.
    ...
    Instead of a spectral decomposition, GMW modifies the diagonal pivots on the fly during the $\mathrm{LD}\mathrm{L}^\top$ factorization.
    At the $j$-th step, given the trial pivot $c_{jj}$ and the largest current off-diagonal $\theta_j = \max_{i>j}|c_{ij}|$, the pivot is bounded from below by
    \begin{equation*}
        d_j^I = \max\!\left(\epsilon,\; |c_{jj}|,\; \frac{\theta_j^2}{\beta^2}\right),
        \qquad
        \beta^2 = \max\!\left(\gamma,\; \frac{\xi}{\sqrt{n^2 - 1}},\; \epsilon_M\right),
    \end{equation*}
    where $\gamma = \max_i |a_{ii}|$, $\xi = \max_{i \neq j} |a_{ij}|$, and $\epsilon_M$ is the machine precision.
    The term $\theta_j^2 / \beta^2$ bounds the entries of $L$ and thus the condition number of the factor, while the implied correction is $e_j = d_j^I - c_{jj} \geq 0$, giving $A + E = L D_I L^\top$.
...
The full hessian regularization aims at the positive definiteness of the Hessian matrix $\tilde{H}$.
...
\end{equation}
where $E_k$ is could be a diagonal matrix or a full matrix, depending on the regularization strategy mentioned in \ref{subsec:reg_strategies}.

The full Hessian regularization is direct and usually simple to implement, 
but it can be too conservative without considering the definiteness of the reduced Hessian $\tilde{M}$, thus slow down the convergence.

\subsection{Reduced Hessian regularization}
According to a well-known result regarding the convexity of QPs, 
which is also mentioned in \cite{verschueren2017sparsity} (for proof, see \cite{nocedal2006numerical}):
\begin{theorem}
...
\end{equation}
\end{theorem}
This characterization shows that the positive definiteness of the reduced Hessian is a necessary and sufficient condition for the QP admitting a unique global optimum.

With the Theorem \ref{thm:reduced_hessian_convexity},
it is reasonable to regularize the reduced Hessian $\tilde{M}$ instead of the full Hessian $\tilde{H}$,
which is less conservative than the full Hessian regularization, while still effective.
And from a perspective of full Hessian regularization in \eqref{eq:regularized_system}, 
the adjustment matrix is $E_k= Z_k E_k^r Z_k^\top$, 
where $E_k^r$ is a positive definite matrix, ;added to regularize the reduced Hessian $\tilde{M}$.
...
second, Riccati recursion is used for regularizing the reduced Hessian $\tilde{M}$ stage-wisely to ensure a positive definite reduced Hessian $\tilde{M}$.
...
can be regarded as the block $\mathrm{LD}\mathrm{L}^\top$ factorization of reduced Hessian $\tilde{M}^i$ in \eqref{eq:schur_comp_KKT_reduced},
...
From the perspective of the reduced Hessian $\tilde{M}^i$ in \eqref{eq:schur_comp_KKT_reduced}, 
eliminating the states through the dynamics leaves a dense reduced Hessian $\tilde{M}^i$ in the inputs $u_0, \dots, u_{N-1}$, 
...
The backward pass in Algorithm~\ref{alg:riccati_recursion} is exactly a block $\mathrm{LD}\mathrm{L}^\top$ factorization of this $\tilde{M}^i$, 
...
so that the pivot block $\tilde{M}_k$ is precisely the Schur complement of stage $k$ in $\tilde{M}^i$ after all later stages have been eliminated. 
...
from stage $N-1$ down to stage $0$, the block $\mathrm{LD}\mathrm{L}^\top$ factorization of the reduced Hessian $\tilde{M}^i$ can be expressed as:
\begin{align*}
    \tilde{M}^i &= L
    \begin{bmatrix}
...
This establishes the connection between positive definiteness of the reduced Hessian $\tilde{M}^i$ and the positive definiteness of the block reduced Hessians $\tilde{M}_k$ obtained from Riccati recursion, 
...
With this connection, we can regularize the reduced Hessian $\tilde{M}^i$ without explicitly computing it, 
...
This can be regarded as a detection of the indefiniteness of the reduced Hessian $\tilde{M}^i$.
And when the Cholesky factorization fails, which indicates that the reduced Hessian $\tilde{M}^i$ is indefinite,
we can regularize the reduced Hessian block $\tilde{M}_k^i$ by adding a block adjust matrix $E_k^i$, i.e. $\tilde{M}_k^i + E_k^i$.
The $E_k^i$ can be computed from the methods mentioned in \ref{subsec:full_hessian}.
...
the block $\mathrm{L}^\top \mathrm{DL}$ factorization of $\tilde{M}^i$,
enforcing the positive definiteness of every $\tilde{M}_k$ during the backward pass is equivalent to regularizing the reduced Hessian $\tilde{M}^i$ itself, 
...
In this subsection, we will show that the minimum perturbation strategy, 
i.e. eigenvalue modification with the minimum Frobenius norm of the adjustment matrix $E_k$ \cite{nocedal2006numerical}, 
...
we can take the absolute value of the negative eigenvalues or inertia as the perturbation magnitude, 
which can avoid the severe condition number issue in the preceding stages. 
The technic is also mentioned in \cite{verschuerenTimeoptimalRaceCar2016b} as MIRROR for eigenvalue modification, 
and \cite{fangModifiedCholeskyAlgorithms2007} as Type-II for modified Cholesky factorization.
...
\For{$k = N-1$ \textbf{down to} $0$}
    ...
    \State $L_k \gets \mathrm{Cholesky}(\tilde{M}_k)$
    \If{factorization fails}
        \State $E_{k+1,P} \gets \mathcal{E}(P_{k+1})_{\lambda} - P_{k+1}$ \Comment{$\mathcal{E}(\cdot)$: MIRROR or Type-II strategy}
        \State $P_{k+1}^\mathrm{reg}$ \gets $P_{k+1}$ + $E_{k+1,P}$ \Comment{Regularize cost-to-go matrix}
        \State $\tilde{M}_k \gets \tilde{R}_k + B_k^\top P_{k+1}^\mathrm{reg} B_k$
        \State $E_{k,M} \gets \mathcal{E}(\tilde{M}_k)_{\lambda} - \tilde{M}_k$
        \State $\tilde{M}_k \gets \tilde{M}_k + E_{k,M}$ \Comment{Regularize reduced Hessian block}
        \State $L_k \gets \mathrm{Cholesky}(\tilde{M}_k)$
    \EndIf
...
The term $E_{k+1,P}$ proactively convexifies the cost-to-go matrix that enters the current stage, 
...
In contrast, $E_{k,M}$ acts as a local safeguard on the reduced Hessian block and is to ensure that the Cholesky factorization succeeds.
Both perturbations are constructed from the absolute values of the negative eigenvalues (the mirror strategy) rather than the minimum-modification rule, 
...
We will start with the regularized KKT formulation where the null space method is applied before the Schur complement to the \eqref{eq:IPM_KKT_system},
...
Proposition~\ref{prop:vanish} shows the regularization is asymptotically inactive: once the iterates are close to $(y^*,\lambda^*)$, $E^i=0$ and the regularized recursion coincides

...
In the collect step, QPs are all with one failing status code from \texttt{acados}, code \texttt{4} for infeasible QP,
which indicating the indefinite reduced Hessian causing the failing of the Riccati recursion in \texttt{HPIPM}.
...
The QPs are stored in \texttt{json} format and are available at \url{https://github.com/Pandatheon/Primal-dual_IPM.git}.
...
Problems that are indefinite in only a small fraction of their stages, such as \textsl{Crane},
\textsl{Flying robot} and \textsl{Furuta}, nevertheless reach very deep negative curvature:
...
Conversely, problems with a high indefinite-stage rate, \textsl{Dubins car} and \textsl{Robot arm},
...
Some problems also fall in the middle, such as \textsl{Moonlander} and
\textsl{Acrobot}, which occupy intermediate indefinite-stage rates
(roughly $0.07$--$0.53$). \textsl{Moonlander} keeps its depth in a narrow band
($|\lambda_{\min}|$ between $10^{0}$ and $10^{2}$), whereas \textsl{Acrobot}
...
from narrow-but-deep (\textsl{Crane}, \textsl{Flying robot}, \textsl{Furuta}),
through intermediate (\textsl{Moonlander}, \textsl{Acrobot}),
to broad-but-moderate (\textsl{Dubins car}, \textsl{Robot arm}),
with \textsl{Pendulum} adding cases of very shallow negative curvature
...
(\textsl{Dubins car}, \textsl{Robot arm}) are indefinite across the majority of their stages.
...
and only \textsl{Acrobot} and \textsl{Crane} are genuinely mixed.

As observed from Figure~\ref{fig:indefinite_source_signature},
most of the problems are governed by the \emph{both} case, 
including \textsl{Moonlander} ($96\%$), \textsl{Robot arm} ($92\%$), \textsl{Dubins car} ($76\%$)
and \textsl{Furuta} ($72\%$).
Notably, three problems, \textsl{Dubins car} ($n=581$), \textsl{Robot arm} ($n=404$) and \textsl{Moonlander} ($n=198$), contribute by far the most indefinite stages, 
they largely account for the aggregate dominance of the \emph{both} case reported above. 

The \emph{P only} case dominates the remaining problems: \textsl{Flying robot} is indefinite purely through the cost-to-go block ($100\%$),
and \textsl{Pendulum} nearly so ($87\%$). The two mixed problems lean the same way:
\textsl{Crane} splits into $58\%$ \emph{P only} against $39\%$ \emph{both},
and \textsl{Acrobot} into $54\%$ \emph{P only} against $41\%$ \emph{both},
...
it appears in only four problems and is negligible for \textsl{Robot arm}, \textsl{Acrobot} and \textsl{Crane} (all below $5\%$), 
with \textsl{Dubins car} being the sole problem where isolated $R$-block indefiniteness is substantial ($24\%$ of its stages,

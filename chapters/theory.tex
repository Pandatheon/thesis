...
the complemented reduced Hessian $\tilde{M}_i = Z^\top \tilde{H}_i Z$ and the right-hand side $Z^\top \tilde{r}_{\mathrm{stat}}^i + Z^\top \tilde{H}_i Y \Delta y_y$.
This formulation exposes the connection between the reduced Hessian $\tilde{M}_i$ and the original Hessian $\tilde{H}$.
...
\label{sec:riccati_recursion}
    ...
    \end{algorithmic}
    \end{algorithm}
    By applying the Riccati recursion, we implicitly solve the KKT system \eqref{eq:schur_comp_KKT_reduced} without explicitly forming the reduced Hessian $\tilde{M}_i$,
    and the computational complexity is reduced from $\mathcal{O}(N^3)$ to $\mathcal{O}(N)$, where $N$ is the horizon length.
...
\label{sec:update}
    ...
    \end{equation}
    where $\mu_i$ is the duality measure, $n_c$ is the number of inequality constraints, 
    ...
    \begin{align}
        ...
        \mu_{\mathrm{aff}} &= \frac{(t_i + \alpha_{\mathrm{prim}}^{\mathrm{aff}} \Delta t_{\mathrm{aff}})^\top (\lambda_i + \alpha_{\mathrm{dual}}^{\mathrm{aff}} \Delta \lambda_{\mathrm{aff}})}{n_c},
    \end{align}
    where the step size $\alpha_{\mathrm{prim}}^{\mathrm{aff}}$ and $\alpha_{\mathrm{dual}}^{\mathrm{aff}}$ are determined 
    by ensuring that the updated slack variables and Lagrange multipliers remain positive, 
    i.e.
    \begin{align}
        \alpha_{\mathrm{prim}}^{\mathrm{aff}} &= \min\left(1, \min_{j:\Delta t_{\mathrm{aff},j}<0} -\frac{t_j^i}{\Delta t_{\mathrm{aff},j}}\right), \\
        \alpha_{\mathrm{dual}}^{\mathrm{aff}} &= \min\left(1, \min_{j:\Delta \lambda_{\mathrm{aff},j}<0} -\frac{\lambda_j^i}{\Delta \lambda_{\mathrm{aff},j}}\right).
    \end{align}
...
\label{sec:size_control}
    ...
    Then, a two-phase line search with backtracking is proposed to ensure progress in either the objective function or the constraint violation, 
    ...
    A two-phase line search inspired by the funnel method \cite{kiesslingUnifiedFunnelRestoration2025} is applied on top of the FTB step size.
    Denoting by $\theta = \lVert c \rVert$ the constraint violation at the current iterate and by $\rho = \nabla f^{\top} d$ the directional derivative of the objective along the search direction $d$,
    the \emph{switching condition}:
    \begin{equation}
        \label{eq:switching}
        \alpha_\mathrm{prim}(-\rho) > \kappa_s\,\theta^{2}, \qquad \kappa_s > 0,
    \end{equation}
    decides which of the two phases the trial step belongs to.
    Because \eqref{eq:switching} involves only quantities of the current iterate and the trial step size, it is evaluated \emph{before} a candidate is formed.
    ...
    so $\theta(\alpha_\mathrm{prim}) = (1-\alpha_\mathrm{prim})\,\theta$ holds exactly along the step.
    The constraint violation therefore decreases monotonically in $\alpha_\mathrm{prim}$, and the largest admissible step gives the fastest feasibility progress.

    If \eqref{eq:switching} holds, sufficient objective progress is expected and the iterate is in the \emph{f-phase}.
    A candidate iterate $z^{+}(\alpha_\mathrm{prim})$ is formed in the primal variables $(x,u,s)$,
    its objective $f^{+}$ is evaluated, and the step is accepted when the Armijo condition \cite{armijo1966minimization}
    \begin{equation}
        \label{eq:armijo}
        f^{+} \le f + \gamma\,\alpha_\mathrm{prim}\,\rho, \qquad \gamma \in (0,1),
    \end{equation}
    is satisfied.
    Otherwise the step size is contracted, $\alpha_\mathrm{prim} \gets \beta\,\alpha_\mathrm{prim}$ with $\beta \in (0,1)$, and the search repeats.
    Contraction shrinks the left-hand side of \eqref{eq:switching}, so a trial that starts in the f-phase may switch to the h-phase after a few backtracks and terminate there.
    If all $n_\mathrm{max}$ trials are rejected, the search reverts to the original FTB step size.
    The dual step size $\alpha_\mathrm{dual}$ is left untouched throughout, as the merit function reads only the primal variables.
    ...
    \begin{algorithmic}[1]
    \State \textbf{Input:} iterate $z$, direction $d$, FTB steps $\alpha_\mathrm{prim},\alpha_\mathrm{dual}$, $f$, $\theta=\lVert c\rVert$, $\rho=\nabla f^{\top}d$
    \State $\alpha_{\mathrm{prim}}^{\mathrm{FTB}} \gets \alpha_\mathrm{prim}$
    \For{$\text{ls} = 1$ \textbf{to} $n_\mathrm{max}$}
        \If{$\alpha_\mathrm{prim}(-\rho) \le \kappa_s\,\theta^{2}$} \Comment{h-phase: feasibility dominates}
            \State \textbf{break} \Comment{accept FTB-scaled step}
        \EndIf
        \State form candidate $z^{+}(\alpha_\mathrm{prim})$
        \State compute $f^{+}$
        \If{$f^{+} \le f + \gamma\,\alpha_\mathrm{prim}\,\rho$} \Comment{f-phase: Armijo}
            \State \textbf{break}
        \EndIf
        \If{$\text{ls} = n_\mathrm{max}$}
            \State $\alpha_\mathrm{prim} \gets \alpha_{\mathrm{prim}}^{\mathrm{FTB}}$ \Comment{all trials rejected: revert to the FTB step}
        \Else
            \State $\alpha_\mathrm{prim} \gets \beta\,\alpha_\mathrm{prim}$
        \EndIf
    \EndFor
    \State \Return $\alpha_\mathrm{prim},\ \alpha_\mathrm{dual}$
    \end{algorithmic}
    \end{algorithm}

    Two simplifications distinguish this scheme from the funnel method of \cite{kiesslingUnifiedFunnelRestoration2025}.
    First, no funnel bound $\theta_{\max}$ is maintained.
    In the funnel method, a trial iterate is admissible only if it stays inside the relaxed feasible set $\{z : \theta(z) \le \theta_{\max}\}$, and the bound is contracted whenever a feasibility step is accepted.
    Here the constraints are affine, so $\theta(\alpha_\mathrm{prim}) = (1-\alpha_\mathrm{prim})\theta \le \theta$ for every $\alpha_\mathrm{prim} \in (0,1]$:
    no trial step can increase the constraint violation, the funnel membership test is satisfied by construction and carries no information, and the h-phase acceptance test and the funnel update become redundant.

    Second, the switching condition \eqref{eq:switching} is built on the first-order term $-\nabla f^{\top} d$ alone, in line with the Armijo condition \cite{armijo1966minimization},
    whereas \cite{kiesslingUnifiedFunnelRestoration2025} uses the second-order model reduction $\Delta m_f = -\tfrac12 d^{\top} H d - \nabla f^{\top} d$.
    ...
    To compensate for dropping the second-order term, the constant $\kappa_s$ is set to a small value, so that objective-reducing steps are not misclassified as h-phase steps.
